...
class _TodayPageState extends State<TodayPage> {
  ...
  Future<void> fetchData() async {
    ...
    if (response.statusCode == 200) {
      ...
      var customerDataFromAPI = itemListJson.where((item) {
        if (item['lastTransactionDate'] != null) {
          DateTime lastTransactionDate =
              DateTime.parse(item['lastTransactionDate']);
          return lastTransactionDate.isAfter(today) &&
              lastTransactionDate.isBefore(today.add(Duration(days: 1)));
        }
        return false;
      }).toList();

      setState(() {
        allCutomerList = customerDataFromAPI
            .map((itemJson) => Item.fromJson(itemJson))
            .toList();
        filteredList = allCutomerList;
        filteredCustomerData = customerDataFromAPI;
        allCutomerData = customerDataFromAPI;
      });
    }
